...
{
	__async_syscall(SYS_rt_sigprocmask, SIG_BLOCK, &all_mask, set, _NSIG/8);
}

void __block_app_sigs(void *set)
{
	__async_syscall(SYS_rt_sigprocmask, SIG_BLOCK, &app_mask, set, _NSIG/8);
}

void __restore_sigs(void *set)
{
	__async_syscall(SYS_rt_sigprocmask, SIG_SETMASK, set, 0, _NSIG/8);
}
